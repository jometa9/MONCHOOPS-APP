  ...
  return (
    <div ref={containerRef} className={cn('relative inline-block', className)}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          'inline-flex h-8 min-w-[7rem] items-center justify-between gap-2 border border-border bg-background px-2 text-xs text-foreground outline-none transition-colors',
      ...
      {open ? (
        <ul
          role="listbox"
          className={cn(
            'absolute right-0 z-50 mt-1 min-w-full max-w-[16rem] overflow-hidden border border-border bg-background py-1 shadow-md',
            menuClassName
          )}
        >
          {options.map((opt) => {
